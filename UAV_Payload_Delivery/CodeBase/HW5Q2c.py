...
for i in range ( n_steps ):
    ...
    if i == 0:
        yaw_estimate[i] = (yaw_data[i] + 0) / dT + vdata[i]
    else:
        yaw_estimate[i] = (yaw_data[i] + yaw_data[i-1])/dT + vdata[i]

    if i == 0:
        yaw_estimate[i] = Accumulator + ((0 + yaw_estimate[i]) * 0.5 * dT)
    else:
        yaw_estimate[i] = Accumulator + ((yaw_estimate[i-1] + yaw_estimate[i]) * 0.5 * dT)
