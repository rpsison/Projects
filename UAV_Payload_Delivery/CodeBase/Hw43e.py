...
sinOmegacT = [math.sin(i*dT*omegac) for i in range ( n_steps )]
...
for i in range ( n_steps ):
    ...
    if Iterm > 50:
        Iterm = 50
    rudderData[i] = Kp * (math.sin(omegac * i) - yaw_data[i]) + (Iterm * (math.sin(omegac * i) - yaw_data[i]))


    u = 0  + rudderData[i] * ((VPC.CndeltaR)/(VPC.Cnbeta))
...
ax3.plot ( t_data , sinOmegacT , label = " sin(omega_c t) ")
